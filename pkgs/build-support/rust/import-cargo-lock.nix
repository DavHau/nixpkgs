...
let
  ...
  parseGit =
    ...
    let
      ...
      key = "__TYPE__:${type}__VALUE__:${value}";
    in
    if parts == null then
      null
    else
      ...
      // lib.optionalAttrs (type != null) { inherit type value key; };
  ...
  # some of its work.
  bultinGitFetched = builtins.listToAttrs (
    builtins.map builtinGitFetch gitDepPackages
  );
